...
For example, consider a dataset with 100 OTUs. The total number of pairwise associations is 4,950. Suppose 500 of them are above the RMT cutoff and count as potential links. Let the corresponding total and direct association strength be $w_i$ and $v_i~(i=1,2,\ldots,500)$, respectively. Using $w_i$ and $v_i~(i=1,2,\ldots,500)$, I can calculate the indirect association strength for the remaining 4,450 association pairs. Let the corresponding total and indirect association strength be $w_i$ and $u_i~(i=501,502,\ldots,4950)$, respectively. The difference $v_i = |w_i \ominus u_i|~(i=501,502,\ldots,4950)$ can be used to estimate the background noise, where $\ominus$ is the inverse operator defined in Eq. (B2) in the supplement (page 21, above line 419). Then we can use the 5% quantile of $v_i~(i=501,502,\ldots,4950)$ as a cutoff, $v_t_h$. The top 500 links with $v_i < v_{th}$ are discarded, since their direct association strength is much smaller than the background noise. Because $v_{th}$ is usually smaller, one may even use $w_i$ instead of $v_i~(i=501,502,\ldots,4950)$ when determining $v_{th}$ to save computation time.
